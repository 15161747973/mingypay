...
class Auth
{
     ...
     * @return boolean
     */
    public function register($username, $password,$auth_code,$refer_id='', $email = '', $mobile = '', $qq, $is_verify_email, $extend = [])
    {
        // 检测用户名或邮箱、手机号是否存在
        if (User::getByUsername($username)) {
            $this->setError('用户名以存在！');
            return false;
        }
        if ($email && User::getByEmail($email)) {
            $this->setError('邮箱号已存在！');
            return false;
        }
...
//        halt($user);
        ...
        if($user->is_freeze == 1){
            $this->setError('账号已被冻结，请联系客服');
            return false;
        }

        if($user->is_check == 0){
            $this->setError('请联系客服人员审核通过方可登录');
            return false;
        }
        //直接登录会员
        $this->direct($user->id);
