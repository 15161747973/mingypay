...
//        $this->acsClient = null;
    ...
    public static function getAcsClient() {
        ...
        $accessKeyId = "LTAIZMwNbeFIGys2"; // AccessKeyId

        $accessKeySecret = "BaNiMCcMgR48oaHHynMGTVrozXbKBs"; // AccessKeySecret
     ...
     * @return stdClass
     */
    public function sendSms($mobile, $templateParam, $type)
    {
        //获取成员属性
        $accessKeyId = $this->accessKeyId;
        $accessKeySecret = $this->accessKeySecret;
        $signName = $this->signName;
        $templateCode = self::get_sms_temp_id($type);
        ...
        // 可选，设置流水号
        $request->setOutId(rand(1000000,9999999));
...
//        halt($acsResponse);
     ...
     * @return stdClass
     */
    public static function sendBatchSms($phone_arr, $param, $type) {
        ...
        $request->setSignNameJson(json_encode(array("明宇科技"), JSON_UNESCAPED_UNICODE));
...
//        halt($info);
        return $info['sms_tpl_code'];

        switch ($type) {
            case '1':           // 注册
                $temp_id = 'SMS_113456069';
                break;
            case '2':           // 买家购买通知
                $temp_id = 'SMS_127168403';
                break;
            case '3':           // 商户密码找回
                $temp_id = 'SMS_128790371';
                break;
            case '4':           //商户库存不足提示
                $temp_id = 'SMS_135029755';
                break;
            case '5':           //安全模式解除动态密码
                $temp_id = 'SMS_135806956';
                break;
            case '6':           //删除收款方式
                $temp_id = 'SMS_169113723';
                break;
            case '7':           //修改绑定手机号
                $temp_id = 'SMS_170155222';
                break;
        }
        return $temp_id;
    }
...
//$SmsDemo = new SmsDemo();
//$response = $SmsDemo->sendSms(15161747973, ['code'=>'1234'], '1');
//echo "发送短信(sendSms)接口返回的结果:\n";
//print_r($response);

//sleep(2);
//
//$response = $SmsDemo->sendBatchSms($mobile, $templateParam, $type);
